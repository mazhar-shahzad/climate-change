...
WTP <- read_excel(
  "doing-economics-datafile-working-in-excel-project-11.xlsx", sheet = "Data")
```
...
# Create the plot
p <- ggplot(df.plmin, aes(WTP_plmin_euro, n)) +
  geom_bar(stat = "identity", position = "identity", fill = "#0072B2") + 
...
# Create the plot
p <- ggplot(df.plmax, aes(WTP_plmax_euro, n)) +
  geom_bar(stat = "identity", position = "identity", fill = "#0072B2") + 
...
p <- ggplot(WTP_DC, aes(y = prop_yes, x = costs)) +
  geom_point(color = "orange", size = 3) +  # Adjust color and size of the points
  geom_line(color = "orange", size = 1) +    # Adjust color and size of the line
  ylab("% Voting 'Yes'") + 
  xlab("Amount (euros)") +
  scale_x_continuous(breaks = seq(0, 1500, 100)) +
  theme_minimal() +  # Change the plot theme
  ggtitle("Voting 'Yes' proportion by Cost") +  # Add plot title
  theme(axis.text = element_text(size = 12),      # Adjust x and y-axis text size
        axis.title = element_text(size = 14),     # Adjust x and y-axis title size
        plot.title = element_text(size = 18, face = "bold"),  # Adjust plot title size and style
        panel.grid.major = element_blank(),       # Remove major gridlines
        panel.grid.minor = element_blank(),       # Remove minor gridlines
        panel.border = element_blank(),           # Remove plot border
        axis.line = element_line(color = "black"), # Add x and y-axis lines
        plot.background = element_blank(),        # Set plot background to blank
...
```{r}
# Data manipulation (assuming you have WTP_DC loaded)
...
WTP_DC %>%
  ...
  scale_color_manual(values = c("blue", "orange"),
                     ...
                     name = "Treatment for 'Abstain' responses") +  # Add a legend title
  ylab("% Voting 'Yes'") + 
  xlab("Costs (euros)") +
  theme_minimal() +  # Use a minimal theme
  theme(plot.title = element_text(size = 10, face = "bold", hjust = 0.5),  # Center the plot title
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 14),
        legend.position = "bottom",   # Set legend position
        legend.text = element_text(size = 12),    # Adjust legend text size
        legend.title = element_text(size = 14),   # Adjust legend title size
        panel.grid.major = element_line(color = "lightgray", linetype = "dashed"),  # Add dashed gridlines
        panel.grid.minor = element_blank(),       # Remove minor gridlines
        panel.border = element_blank(),           # Remove plot border
        plot.background = element_blank()         # Set plot background to blank
  )
